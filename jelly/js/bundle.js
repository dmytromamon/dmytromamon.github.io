...
/* harmony import */ var jquery__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(jquery__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var gsap__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! gsap */ "./node_modules/gsap/index.js");
/* harmony import */ var jelly__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! jelly */ "./src/js/jelly.js");
...
/* harmony import */ var vendor_smooth_scrollbar_SoftScrollPlugin__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! vendor/smooth-scrollbar/SoftScrollPlugin */ "./src/js/vendor/smooth-scrollbar/SoftScrollPlugin.js");
...
/* harmony import */ var cursor__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! cursor */ "./src/js/cursor.js");
/* harmony import */ var magnetic__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! magnetic */ "./src/js/magnetic.js");
/*** IMPORTS FROM imports-loader ***/var define=false;// if you want support IE11
var tl=gsap__WEBPACK_IMPORTED_MODULE_1__["default"].timeline();if(jquery__WEBPACK_IMPORTED_MODULE_0___default()(window).innerWidth()<1080){tl.staggerTo('.card',.2,{ease:"power3.inOut",scale:1,opacity:1},.025);}else{if(jquery__WEBPACK_IMPORTED_MODULE_0___default()('.card-container').length>0){for(var i=0;i<jquery__WEBPACK_IMPORTED_MODULE_0___default()('.card-container').length;i++){var container='.card-container:nth-child('+(i+1)+')';var item=container+' .card';var label="-=.225";if(i==0){label=0;}tl.staggerTo(item,.2,{ease:"power3.inOut",scale:1,opacity:1},.025,label);}}}// Soft edges plugin for SmoothScroll
smooth_scrollbar__WEBPACK_IMPORTED_MODULE_3__["default"].use(vendor_smooth_scrollbar_SoftScrollPlugin__WEBPACK_IMPORTED_MODULE_4__["default"]);// Init smooth scrollbar
var view=document.getElementById('view-main');var scrollbar=smooth_scrollbar__WEBPACK_IMPORTED_MODULE_3__["default"].init(view,{renderByPixels:false,damping:0.075});// Init Jelly and provide smooth scrollbar offset
var jelly=new jelly__WEBPACK_IMPORTED_MODULE_2__["default"]({scrollPos:function scrollPos(){return scrollbar.offset.y;}});// Optional demo: pause when scroll via scrollbar track
scrollbar.track.yAxis.element.addEventListener('mousedown',function(){jelly.pause(true);});document.documentElement.addEventListener('mouseup',function(){jelly.pause(false);},true);// Init cursor
var cursor=new cursor__WEBPACK_IMPORTED_MODULE_6__["default"]();// Init magnetic
jquery__WEBPACK_IMPORTED_MODULE_0___default()('[data-magnetic]').each(function(){new magnetic__WEBPACK_IMPORTED_MODULE_7__["default"](this);});
...
/*!*************************!*\
  !*** ./src/js/jelly.js ***!
  \*************************/
/*! exports provided: default, Jelly */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return JellyEffect; });
 ...
 */var JellyEffect=/*#__PURE__*/function(){function JellyEffect(options){_classCallCheck(this,JellyEffect);this.jellys=[];this.els=document.querySelectorAll('[data-cuberto-jelly]');for(var i=0;i<this.els.length;i++){var el=this.els[i];var opts=Object.assign({},options,el.dataset.cubertoJelly?JSON.parse(el.dataset.cubertoJelly):{});var jelly=new Jelly(el,opts);this.jellys.push(jelly);}}_createClass(JellyEffect,[{key:"pause",value:function pause(state){for(var i=0;i<this.jellys.length;i++){this.jellys[i].pause(state);}}},{key:"destroy",value:function destroy(){for(var i=0;i<this.jellys.length;i++){this.jellys[i].destroy();}}}]);return JellyEffect;}();var Jelly=/*#__PURE__*/function(){function Jelly(el,options){_classCallCheck(this,Jelly);this.el=el;this.paused=false;this.options=Object.assign({},{intensity:0.15,speed:0.6,min:-5,max:5,scrollPos:function scrollPos(){return window.pageYOffset;}},options);this.init();}_createClass(Jelly,[{key:"getScrollPos",value:function getScrollPos(){return this.options.scrollPos();}},{key:"pause",value:function pause(){var state=arguments.length>0&&arguments[0]!==undefined?arguments[0]:true;this.paused=state;}},{key:"init",value:function init(){var _this=this;this.observer=new IntersectionObserver(function(entries){if(entries[0].isIntersecting){_this.startLoop();}else{_this.stopLoop();}});this.observer.observe(this.el);}},{key:"startLoop",value:function startLoop(){this.y=this.getScrollPos();this.loop();}},{key:"stopLoop",value:function stopLoop(){cancelAnimationFrame(this.frame);if(!this.paused){gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(this.el,{skewY:0});}}},{key:"loop",value:function loop(){var y=this.getScrollPos();var diff=y-this.y;var skew=Math.min(Math.max(this.options.min,diff*this.options.intensity),this.options.max);if(!this.paused){gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(this.el,{skewY:skew,force3D:true});this.animationPaused=false;}else{if(!this.animationPaused){gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(this.el,{skewY:0,force3D:true,duration:this.options.speed});this.animationPaused=true;}}this.y=y;this.frame=requestAnimationFrame(this.loop.bind(this));}},{key:"destroy",value:function destroy(){this.observer.disconnect();this.pause(false);this.stopLoop();}}]);return Jelly;}();
