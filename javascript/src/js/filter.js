  $('.on_target_home').each(function(i) {
    ...
    if ((+$('.minutes_value_one').eq(0).text() > $('.time').eq(i).text().slice(0,-1)) || (+$('.minutes_value_two').eq(0).text() < $('.time').eq(i).text().slice(0,-1))) {
      $('.time').eq(i).closest('.can-bulb').css('display', "none");          
    }
    if (+$('.minutes_value_two').eq(0).text() == 90) {      
      if (125 < $('.time').eq(i).text().slice(0,-1)) {
        $('.time').eq(i).closest('.can-bulb').css('display', "none");          
      }         
    }
  });
...
$('#button-reset').click(function (e){
  $.ajax({
    url: "filter-reset.php",
    cache: false,
    success: function(data){
      $(".filter-container").empty();
      $(".filter-container").prepend($(data));
